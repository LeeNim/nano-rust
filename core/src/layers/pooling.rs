...
impl Layer for MaxPool2DLayer {
    ...
    fn output_shape(&self, input_shape: &Shape) -> NanoResult<Shape> {
        ...
        let (out_h, out_w) = math::pool_output_size(in_h, in_w, self.pool_h, self.pool_w, self.stride);
    ...
    fn forward<'a>(
        ...
        math::max_pool_2d(input, output, channels, in_h, in_w, self.pool_h, self.pool_w, self.stride)?;
        Ok((output, out_shape))
    }
}
